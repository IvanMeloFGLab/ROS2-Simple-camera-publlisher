...
public:
    ...
    : Node("yolo11") {
        ...
        cv::namedWindow("Yolo");
        first_ = true;
        hef_file_ = (ament_index_cpp::get_package_share_directory("cpp_camera") + "/yolos/x.hef");
...
private:
    ...
    hailo_status infer(std::vector<InputVStream> &input, std::vector<OutputVStream> &output, cv::Mat frame_i) {
        try {
            ...
            frame = Yolo11::letterbox(frame, 640);
    ...
    static constexpr std::array<const char*, 1> LABELS = {
        "ball"
    };
